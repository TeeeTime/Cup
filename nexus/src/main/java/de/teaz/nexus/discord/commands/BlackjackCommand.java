...
import de.teaz.nexus.games.blackjack.Blackjack;
import de.teaz.nexus.games.blackjack.Card;
import de.teaz.nexus.games.blackjack.BlackjackGameState;
...
public class BlackjackCommand extends ListenerAdapter implements Command {
	...
	public void execute(MessageReceivedEvent event) {
		...
		if(blackjack.getGameState() == BlackjackGameState.INSTANTBLACKJACK) {
		...
		}else if(blackjack.getGameState() == BlackjackGameState.TIE) {
		...
		}else if(blackjack.getGameState() == BlackjackGameState.DEALERWIN) {
			DiscordBot.INSTANCE.getBlackjackManager().removeBlackjack(event.getAuthor().getId());
			Stats.incrementStat("blackjackPlayed", event.getAuthor());
	...
	public void onButtonInteraction(ButtonInteractionEvent event) {
		...
		if(game.getGameState() == BlackjackGameState.DEALERWIN) {
		...
		}else if(game.getGameState() == BlackjackGameState.INSTANTBLACKJACK) {
			CoinManager.setCoins(event.getUser().getId(), CoinManager.getCoins(event.getUser().getId()) + (game.getBet() * 2) + (game.getBet() / 2));
			DiscordBot.INSTANCE.getBlackjackManager().removeBlackjack(event.getUser().getId());
			
		}else if(game.getGameState() == BlackjackGameState.PLAYERWIN) {
		...
		}else if(game.getGameState() == BlackjackGameState.TIE) {
	...
	private EmbedBuilder getGameEmbed(Blackjack blackjack, User user) {
		...
		if(blackjack.getGameState() == BlackjackGameState.DEALERWIN) {
			embedText += "\nYou lost " + blackjack.getBet() + " :coin:";
			
		}else if(blackjack.getGameState() == BlackjackGameState.INSTANTBLACKJACK) {
			embedText += "\nYou won " + ((blackjack.getBet() * 2) + (blackjack.getBet() / 2)) + " :coin:";
			
		}else if(blackjack.getGameState() == BlackjackGameState.PLAYERWIN) {
